...
lli romanToInt(char val[])
{
    lli length = strlen(val);
    lli intVal = 0, i;
    for (i = 0; i < length - 1; i++)
    {
        lli a = getValue(val[i]), b = getValue(val[i + 1]);
        if (a < b)
        {
            intVal += (b - a);
            i++;
...
int main()
{
    lli intarr[] = {1000, 900, 500, 400, 100, 90, 50, 40, 10, 9, 5, 4, 1};
    ...
    while (t--)
    {
        ...
        scs(val1);
        scs(val2);
        lli x = romanToInt(val1);
        lli y = romanToInt(val2);
        lli result;
        prs("1. Addition \n2. Multipilcation \n");
        lli opt;
        sci(opt);
